...
export default function Footer() {
  return (
    <footer
      className={clsx(
        "bg-bg-dark text-white py-10 md:py-10 lg:py-11",
        "xl:py-12 px-4 md:px-6 lg:px-8 xl:px-10",
      )}
    >
      <Container>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 md:gap-8 lg:gap-12 xl:gap-16">
          ...
          <div className="space-y-5">
            ...
            <p
              className={clsx(
                "text-white text-sm md:text-sm lg:text-base 2xl:text-[18px]",
                "leading-relaxed md:max-w-full lg:max-w-80 xl:max-w-100",
              )}
            >
                ...
                <div className="flex flex-col gap-y-2 md:flex-wrap md:flex-row md:gap-x-6">
                  {FOOTER_MENU_LINKS.map((item) => (
                    <Link
                      key={item.label}
                      href={item.href}
                      className="text-white text-[14px] xl:text-base"
                    >
                      {item.label}
                    </Link>
                  ))}
          ...
          <div className="space-y-2 lg:text-right">
            ...
            <div className="md:flex lg:justify-end mt-6">
              <a
                ...
                className={clsx(
                  "inline-flex items-center gap-3 bg-white text-green-primary",
                  "text-[14px] xl:text-base font-semibold pl-5 pr-2 py-2 rounded-xl",
